	...
	avltree_t** new_root)
{
	while(size > 0) {
		...
		read_len = fread(data, data_len, 1, file);
...
errno_t SaveAvlTree(avltree_t* tree, FILE* file, int32_t* size)
{
	...
	errno_t ret = SaveAvlTree(tree->left, file, size);
	if (ret != 0) {
		return ret;
	}
	size_t write_len = 0;
	...
	write_len = fwrite(&tree->max_key, sizeof(tree->max_key), 1, file);
	if (write_len != sizeof(tree->max_key)) {
		return errno;
	}
	write_len = fwrite(&tree->data_len, sizeof(tree->data_len), 1, file);
	if (write_len != sizeof(tree->data_len)) {
		return errno;
	}
	write_len = fwrite(tree->data, tree->data_len, 1, file);
	if (write_len != tree->data_len) {
		return errno;
	}

	*size += sizeof(tree->min_key) + sizeof(tree->max_key) 
		+ sizeof(tree->data_len) + tree->data_len;
	ret = SaveAvlTree(tree->right, file, size);
	if (ret != 0) {
		return ret;
	}
